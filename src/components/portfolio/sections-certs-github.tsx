...
export function Certifications() {
  ...
  return (<section id="certs" className="section-pad relative overflow-hidden">
    <div aria-hidden className="pointer-events-none absolute inset-0 -z-10" style={{
      backgroundImage: "repeating-linear-gradient(135deg, rgba(245,166,35,0.03) 0px, rgba(245,166,35,0.03) 1px, transparent 1px, transparent 40px)",
    }} />
    <div className="mx-auto max-w-6xl px-6">
      <div className="grid grid-cols-1 gap-12 md:grid-cols-12">
        <div className="md:col-span-4">
          <SectionLabel slug="compliance" title="Certifications" />
          ...
          {/* cert count badge */}
          <div className="mt-6 inline-flex items-center gap-2 rounded-md bg-status-good/10 px-4 py-2.5 ring-1 ring-status-good/25">
            <Award className="h-4 w-4 text-status-good" />
            <span className="font-mono text-sm font-medium text-status-good">{certs.length} audits passed</span>
          </div>
        </div>
            ...
            {certs.map((c, i) => (
              <motion.li key={c.name} initial={{ opacity: 0, x: -12 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.06 }}>
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="group flex items-center gap-4 rounded-md hairline bg-surface/60 px-5 py-4 transition hover:bg-surface-elevated hover:ring-1 hover:ring-primary/30"
                >
                  <div className="grid h-10 w-10 shrink-0 place-items-center rounded-md bg-primary/10 text-primary transition group-hover:bg-primary/20 group-hover:shadow-[0_0_16px_-2px_rgba(245,166,35,0.55)]">
                    <Award className="h-5 w-5" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="truncate font-display text-base font-bold tracking-tight sm:text-lg">{c.name}</div>
    ...
    <div className="mx-auto max-w-6xl px-6">
      <div className="grid grid-cols-1 gap-10 md:grid-cols-12">
        ...
        <div className="md:col-span-4 md:sticky md:top-28 md:self-start">
          <SectionLabel slug="activity" title="GitHub" />
          <h2 className="mt-6 font-display text-balance text-4xl font-bold leading-[1.0] tracking-tight sm:text-5xl">Built in<br />the open.</h2>
        ...
        <div className="md:col-span-8 space-y-4">
          ...
          <div>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="mt-3 overflow-hidden rounded-md hairline bg-surface/60 p-4">
              <img src={`https://ghchart.rshah.org/f5a623/${USER}`} alt={`${USER} GitHub contribution chart`} className="w-full rounded" loading="lazy" />
            </motion.div>
          </div>

          {/* profile */}
          <div className="flex flex-wrap gap-3">
            <a href={`https://github.com/${USER}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-md bg-foreground/95 px-5 py-2.5 text-sm font-semibold text-background transition hover:bg-foreground">
